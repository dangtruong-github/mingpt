...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    theta_i = 1.0 / (10000.0 ** (2*(torch.arange(0, dim, 2).float()-1) / dim))
    # print(theta_i.shape) # shape: [dim/2]

    position = torch.arange(0, max_positions, 1).float().unsqueeze(1) # shape: [max_positions, 1]
    t_theta_i = position * theta_i  # shape [max_positions, dim/2]
    # print(t_theta_i.shape)

    sin_t_theta_i = torch.sin(t_theta_i).unsqueeze(-1)
    cos_t_theta_i = torch.cos(t_theta_i).unsqueeze(-1)
    rope_cache = torch.cat((sin_t_theta_i, cos_t_theta_i), axis=2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    # Reshape x to complex: [B, H, S, D/2, 2] → complex [B, H, S, D/2]
    x_reshaped = x.view(*x.shape[:-1], x.shape[-1] // 2, 2)
    x_complex = torch.view_as_complex(x_reshaped)

    # Truncate rope_caches
    rope_cache = rope_cache[:x.shape[2]]  # → [seq_len, head_dim/2, 2]
    rope_complex = torch.view_as_complex(rope_cache).unsqueeze(0).unsqueeze(0)

    # rope_complex has shape [S, D/2] → unsqueeze(0, 0) → [1, 1, S, D/2]
    rotated = x_complex * rope_complex
    rotated = torch.view_as_real(rotated)  # [B, H, S, D/2, 2]
    rotated_x = rotated.view(*rotated.shape[:-2], rotated.shape[-2] * rotated.shape[-1])
