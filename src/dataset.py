...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        new_length = random.randint(4, int(self.block_size*7/8))
        ...
        suffix = chunk[start_masked_idx+length_masked:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content

        final_string = masked_string + (self.PAD_CHAR * (self.block_size + 1 - len(masked_string)))
        
        dix = [self.stoi[s] for s in final_string]
